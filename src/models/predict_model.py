...
@click.command()
@click.option("--context_size", "-cs", type=int,
              help="How much context to use for the"
...
def main(context_size, prefix, seed):
    ...
    try:
        ...
        raise FileNotFoundError("No model trained with context size equal"
                                f"to {context_size}."
                                "Use a context size from 0 to 4.")
